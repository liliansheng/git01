...
function deleteType(typeId){
	...
	swal({ 
		...
		$.ajax({
			...
			success:function(result){  //result 是servlet返回的回调函数结果
				// 判断是否删除成功
				if (result.code == 1) {
					// 提示成功
...
function openUpdateDialog(typeId) {
	...
	//得到要修改记录的类型的ID，（在第一个td中，下标是0）将其设置到模态框中隐藏域中
...
function addDom(typeId,typeName) {
	...
	// a、通过表格的id属性获取表格元素对象
	var myTable = $("#myTable");
	// b、判断表格对象的length是否大于0 （大于0 ，则表示表格存在；否则表格不存在）(这里[maTable]是通过jqury获取的，它不会为null,所以不能跟null比)
	if (myTable.length > 0) {
		// c、如果表格存在，则拼接tr记录，将tr对象追加到table对象中
		myTable.append(tr);
	} else {
		...
		//用直接赋值而不是用追加，因为前面有默认值，用追加，默认值不会消失
		$("#myDiv").html(myTable);
	}
