...
<html lang="zh"><head><meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
    ...
    <%--info.jsp里使用比较方法，因为info页面最终被这个页面包含，引入一个工具js的jar包--%>
        ...
        <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
            ...
            <form class="navbar-form navbar-right" role="search" action="index">
                <div class="form-group">
                    ...
                    <input type="text" name="title" value="${title}" class="form-control" placeholder="搜索云记">
                </div>
            ...
            <div class="data_list">
                ...
                <div class="userimg">
                   <%-- <img src="user?actionName=userHead&imageName=${user.head}">--%>
                    <img src="user?actionName=userHead&headName=${user.head}">
                </div>
                ...
                <div>
                    <ul id = "typeUl" class="nav nav-pills nav-stacked">
                      <c:forEach items="${typeInfo}" var="item">
                        <li id = "${item.typeId}"><a href="index?actionName=searchType&typeId=${item.typeId}"><span id = "sp_1">${item.groupName}</span> <span class="badge">${item.noteCount}</span></a></li>
                      </c:forEach>
                    </ul>
                </div>
