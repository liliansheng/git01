...
public class UserService {
    ...
    public ResultInfo<User> userLogin(String userName,String userPwd){
        ...
        // 1. 判断参数是否为空   (StrUtil.isBlank(判断值)是设定好的方法，通关断是null还是空字符串 )
         if (StrUtil.isBlank(userName) || StrUtil.isBlank(userPwd)){
              // 如果为空
              // 设置ResultInfo对象的状态码和提示信息
             resultInfo.setCode(0);
             resultInfo.setMsg("请输入姓名和密码");
             //返回resultInfo对象
             return resultInfo;
         }
        // 2. 如果不为空，通过用户名查询用户对象
        ...
        //  4. 如果用户对象不为空，将数据库中查询到的用户对象的密码与前台传递的密码作比较 （将前台传的密码进行MD5加密后再比较）
         //将前台传的密码按照MD5算法进行加密
