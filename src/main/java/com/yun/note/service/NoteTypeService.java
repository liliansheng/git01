...
import com.yun.note.dao.NoteTypeDao;
...
public class NoteTypeService {
    ...
    public ResultInfo<Integer> addOrUpdate(String typeId, String typeName, Integer userId) {
        ...
        if (code == 0){
            resultInfo.setCode(0);
            resultInfo.setMsg("类型已存在，请输入其他名称");
            return resultInfo;
        }
        // 3. 判断类型ID是否为空(根据数据库中有没有这个类型数据，来判断是执行修改还是添加)
        Integer key = null;
        if (StrUtil.isBlank(typeId)){
            key = NoteTypeDao.addType(userId,typeName);
        }else {
